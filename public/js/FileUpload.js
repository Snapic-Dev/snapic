...
var FileUpload = {
  ...
  updatePreviewElement: function (file, localFile, attachment = false) {
    ...
    switch (file.type) {
      ...
      case "video":
        ...
        if (localFile) {
          FileUpload.setMediaSourceForPreviewByElementAndFile(
            videoPreviewEl,
            file
          );
        } else {
  ...
  setMediaSourceForPreviewByElementAndFile: function (element, file) {
    ...
    if (
      element.canPlayType(file.type).length &&
      element.canPlayType(file.type) !== "no"
    ) {
  ...
  setPreviewSource: function (element, file, attachment) {
    if (
      attachment.coconut_id !== null &&
      attachment.path.indexOf("videos/tmp/") >= 0
    ) {
    ...
    $.ajax({
      ...
      error: function () {
        launchToast(
          "danger",
          trans("Error"),
          trans("Failed to remove the attachment.")
        );
      },
    });
  },
