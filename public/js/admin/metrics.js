...
let getCardValue = (form, ts) => {
  hideCardElements(
    form.querySelectorAll(
      "[data-card-value], [data-card-status-increase], [data-card-status-decrease], [data-card-status-npd], [data-card-status-ncd], [data-card-status-constant], [data-card-status-error]"
    )
  );
  showCardElements(
    form.querySelectorAll("[data-card-loading], [data-card-status-loading]")
  );
  ...
  fetch(form.action + "?" + data.toString(), {
    ...
    .then((data) => {
      ...
      if (growth > 0) {
        form.querySelector("[data-card-increase-growth]").textContent =
          Number.parseFloat(Math.abs(growth).toFixed(2)).toString();
        showCardElements(form.querySelectorAll("[data-card-status-increase]"));
      } else if (growth < 0) {
        form.querySelector("[data-card-decrease-growth]").textContent =
          Number.parseFloat(Math.abs(growth).toFixed(2)).toString();
      ...
      hideCardElements(
        form.querySelectorAll(
          "[data-card-loading], [data-card-status-loading], [data-card-status-error]"
        )
      );
      showCardElements(form.querySelectorAll("[data-card-value]"));

      form.querySelector("[data-card-value]").textContent = data.value
        .format(0, 3, ts)
        .toString();
    })
    .catch((error) => {
      ...
      form.querySelector("[data-card-status-error]").textContent =
        error.statusText;
    });
};
...
let getCardTrend = (form, chart, ts, st) => {
  hideCardElements(
    form.querySelectorAll(
      "[data-card-value], [data-card-status-error], [data-card-chart]"
    )
  );
  showCardElements(
    form.querySelectorAll("[data-card-loading], [data-card-status-loading]")
  ...
  fetch(form.action + "?" + data.toString(), {
    ...
    .then((data) => {
      ...
      if (st) {
        showCardElements(form.querySelectorAll("[data-card-value]"));
        form.querySelector("[data-card-value]").textContent = total
          .format(0, 3, ts)
          .toString();
      }
    })
...
let getCardPartition = (form, chart, ts, color) => {
  hideCardElements(
    form.querySelectorAll(
      "[data-card-status-error], [data-card-chart], [data-card-legend]"
    )
  );
  showCardElements(
    form.querySelectorAll("[data-card-loading], [data-card-status-loading]")
  ...
  fetch(form.action + "?" + data.toString(), {
    ...
    .then((data) => {
      ...
      for (const [key, value] of Object.entries(data.values)) {
        ...
        legendEl.querySelector("[data-legend-value]").textContent = parseFloat(
          value
        )
          .format(0, 3, ts)
          .toString();
      ...
      showCardElements(
        form.querySelectorAll("[data-card-chart], [data-card-legend]")
      );
