...
$(function () {
  if (messengerVars.bootFullMessenger) {
    ...
    if (
      messengerVars.lastContactID !== false &&
      messengerVars.lastContactID !== 0
    ) {
...
var messenger = {
  ...
  boot: function () {
    Pusher.logToConsole =
      typeof messengerVars.pusherDebug !== "undefined"
        ? messengerVars.pusherDebug
        : false;
    let params = {
  ...
  initLiveSockets: function () {
    ...
    $.each(messenger.state.contacts, function (k, v) {
      ...
      channel.bind("new-message", function (data) {
        ...
        messenger.updateUnreadMessagesCount(
          parseInt($("#unseenMessages").html()) + 1
        );
  ...
  sendMessage: function (forceSave = false) {
    ...
    // Validation
    if (
      $(".messageBoxInput").val().length === 0 &&
      FileUpload.attachaments.length === 0
    ) {
    ...
    $.ajax({
      ...
      success: function (result) {
        ...
        if (messenger.state.receiverIDs.length === 1) {
          ...
          messenger.addLatestMessageToConversation(
            result.data.message.receiverID,
            result.data.message
          );
          if (messenger.state.newConversationMode) {
            messenger.fetchContacts(function () {});
            messenger.state.activeConversationUserID =
              result.data.message.receiver_id;
          ...
          const latestContactId =
            result.data[result.data.length - 1].message.receiver_id;
          if (messenger.state.newConversationMode) {
            messenger.fetchContacts();
          }
    ...
    $(".messageBoxInput").on("click", function () {
      if ($("#unseenValue").val() !== 0) {
        $.ajax({
          ...
          dataType: "json",
          success: function (result) {
            messenger.markConversationAsRead(
              messenger.state.activeConversationUserID,
              "read"
            );
            messenger.updateUnreadMessagesCount(
              parseInt($("#unseenMessages").html()) - result.data.count
            );
  ...
  reloadContactsList: function () {
    ...
    if (messenger.state.contacts.length > 0) {
      $(".conversations-list").html(
        '<div class="row">' + contactsHtml + "</div>"
      );
    } else {
      $(".conversations-list").html(noContactsLabel());
    }
    $(".contact-" + messenger.state.activeConversationUserID).addClass(
      "contact-active"
    );
  },
  ...
  reloadConversationHeader: function () {
    if (typeof messenger.state.conversation[0] !== "undefined") {
      ...
      const userID =
        contact.receiver_id !== messenger.state.activeConversationUserID
          ? contact.sender.id
          : contact.receiver.id;
      const username =
        contact.receiver_id !== messenger.state.activeConversationUserID
  ...
  reloadConversation: function () {
    ...
    if (urlParams.has("token") && !messenger.state.redirectedToMessage) {
      ...
      if (
        $(".conversation-content .message-box").length &&
        $(".conversation-content").find(token).length
      ) {
        let offset =
          $(".conversation-content").find(token).offset().top -
          $(".conversation-content").offset().top +
      ...
      if ($(".conversation-content .message-box").length) {
        $(".conversation-content").animate(
          { scrollTop: $(".conversation-content")[0].scrollHeight + 100 },
          800
        );
      }
    }
  ...
  textAreaAdjust: function (el) {
    el.style.height =
      el.scrollHeight > el.clientHeight ? el.scrollHeight + "px" : "40px";
  },
      ...
      messenger.selectizeInstance = $("#select-repo").selectize({
        ...
        render: {
          option: function (item, escape) {
            return (
              "<div>" +
              '<img class="searchAvatar ml-3 my-1" src="' +
              escape(item.avatar) +
              '" alt="">' +
          ...
          item: function (item, escape) {
            return (
              "<div>" +
              '<img class="searchAvatar ml-1" src="' +
              escape(item.avatar) +
              '" alt="">' +
              '<span class="name ml-2">' +
              escape(item.name) +
  ...
  deleteMessage: function () {
    $.ajax({
      ...
      dataType: "json",
      url:
        app.baseUrl + "/my/messenger/delete/" + messenger.state.activeMessageID,
      success: function (result) {
        let element = $(
          '*[data-messageid="' + messenger.state.activeMessageID + '"]'
        );
        element.remove();
        ...
        if (result.isLastMessage === true) {
          messenger.fetchContacts(function () {
            if (messenger.state.contacts.length >= 1) {
              messenger.state.activeConversationUserID =
                messenger.state.contacts[0].contactID;
              messenger.fetchConversation(
                messenger.state.activeConversationUserID
              );
            } else {
              messenger.fetchContacts();
  ...
  closeNewConversationUI: function () {
    ...
    if (
      messenger.state.contacts.length === 0 &&
      messengerVars.lastContactID === 0
    ) {
      $(".conversation-content").html(noMessagesLabel());
      $(".conversation-writeup").addClass("hidden");
