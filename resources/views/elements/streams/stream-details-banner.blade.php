    ...
    <div class="mr-4 overflow-hidden">
        ...
        <div class="d-flex flex-row my-1">
            <div class="d-flex justify-content-center">
                <img class="rounded-circle rounded-circle-stream avatar" src="{{ $stream->user->avatar }}"
            ...
            <div class="pl-3 w-100 d-flex align-items-center">
                <div>
                    <div class="d-flex flex-column overflow-hidden">
                        <h5 class="text-truncate">
                            {!! __(":user's stream", [
                                'user' =>
                                    "<a href=\"" .
                                    route('profile', ['username' => $stream->user->username]) .
                                    "\" class=\"text-" .
                                    (Cookie::get('app_theme') == null
                    ...
                    @if (!isset($streamEnded))
                        <span class="text-muted">{{ __('Started streaming') }}
                            {{ \Carbon\Carbon::parse($stream->created_at)->diffForhumans() }}.</span>
                    @else
    ...
    @if (!isset($streamEnded))
        <div class="d-flex align-items-center">
            @if (isset($subLocked) && $stream->user->id !== Auth::user()->id)
                <div class="d-none d-sm-block">
                    <span class="p-pill ml-2 pointer-cursor to-tooltip stream-subscribe-button"
                        @if (!\App\Providers\GenericHelperServiceProvider::creatorCanEarnMoney($stream->user)) data-placement="top"
                ...
                data-stream-id="{{ $stream->id }}" @endif>
            ...
            @if (isset($priceLocked) && $stream->user->id !== Auth::user()->id)
                <div class="d-none d-sm-block">
                    <span class="p-pill ml-2 pointer-cursor to-tooltip stream-unlock-button"
                        @if (!GenericHelper::creatorCanEarnMoney($stream->user)) data-placement="top"
                title="{{ __('This creator cannot earn money yet') }}"
                @else
                data-toggle="modal"
                data-target="#checkout-center"
                ...
                <div class="dropdown {{ GenericHelper::getSiteDirection() == 'rtl' ? 'dropright' : 'dropleft' }}">
                    ...
                    <div class="dropdown-menu">
                        <a class="dropdown-item" href="javascript:void(0);"
                            onclick="Lists.showReportBox({{ $stream->user->id }},null,null,{{ $stream->id }});">{{ __('Report') }}</a>
                    </div>
                </div>
            @endif

            <div class="d-none d-sm-block">
                <div class="watchingUsersPill ml-2 pointer-cursor" href="{{ route('my.streams.get') }}?action=details"
                    title="Assistindo">
                    <span class="d-flex textWatchingArea">
                        @include('elements.icon', ['icon' => 'person-outline'])
                        <span class="live-stream-users-count text-sm">0</span>
                    </span>
                </div>
            </div>
            ...
            @if ($stream->user->id === Auth::user()->id)
                <div class="d-none d-sm-block">
                    <a class="p-pill ml-2 pointer-cursor to-tooltip"
                        href="{{ route('my.streams.get') }}?action=details" title="{{ __('Stream details') }}">
