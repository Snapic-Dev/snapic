            ...
            <div class="modal-body">
                ...
                <div class="tab-content" id="nav-tabContent">
                    <div class="tab-pane fade show active" id="nav-desktop" role="tabpanel">
                        <div class="mt-2">
                            <ul class="py-3">
                                <li class="mb-2">{{__('Download')}} <a href="https://obsproject.com/download" target="_blank">OBS</a> {{__('for desktop or mobile alternatives.')}}</li>
                                <li class="mb-2">{{__('Go to')}} <code>{{__("Settings > Stream")}}</code>. {{__('For')}} <code>{{__("Service")}}</code>, {{__('select')}} <code>{{__("Custom")}}</code>.</li>
                                <li class="mb-2">{{ucfirst(__('for the'))}} <code>{{__("Server & Stream key")}}</code>, {{__('use the values below.')}}</li>
                            </ul>
                            <div class="form-group row flex-column p-2 px-3">
                                <label for="colFormLabelSm" class="ml-3">{{__('Stream url')}}</label>
                                <div class="p-2 d-flex">
                                    <input type="text" class="form-control form-control-md ml-2 mr-2" id="stream-url" placeholder="{{__('Stream url')}}">
                                    <span class="h-pill rounded mr-2" onclick="Streams.copyStreamData('url')">
                                        @include('elements.icon',['icon'=>'copy-outline'])
                                    </span>
                                </div>
                            </div>
                            <div class="form-group row flex-column p-2 px-3">
                                <label for="colFormLabelSm" class="ml-3">{{__('Stream key')}}</label>
                                <div class="p-2 d-flex">
                                    <input type="text" class="form-control form-control-md ml-2 mr-2" id="stream-key" placeholder="{{__('Stream key')}}">
                                    <span class="h-pill rounded mr-2" onClick="Streams.copyStreamData('key');">
                                        @include('elements.icon',['icon'=>'copy-outline'])
                                    </span>
                                </div>
                            </div>
                    ...
                    <div class="tab-pane fade" id="nav-mobile" role="tabpanel" >
                        <div class="mt-2">
                            ...
                            <div class="form-group row flex-column p-2 px-3">
                                <label for="colFormLabelSm" class="ml-3">{{__('Stream url')}}</label>
                                <div class="p-2 d-flex">
                                    <input type="text" class="form-control form-control-md ml-2 mr-2" id="stream-url-larix" placeholder="{{__('Stream url')}}">
                                    <span class="h-pill rounded mr-2" onclick="Streams.copyStreamData('mobile-url')">
                                        @include('elements.icon',['icon'=>'copy-outline'])
                                    </span>
                                </div>
                            </div>
