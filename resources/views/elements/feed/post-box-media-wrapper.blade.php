...
<a href="{{$attachment->path}}" rel="mswp" title="">
    ...
    <div class="video-wrapper h-100 w-100 d-flex justify-content-center align-items-center object-cover">
        <video class="video-preview w-100 h-100" src="{{$attachment->filename}}" controls controlsList="nodownload" preload="metadata" style="min-height: 100%;object-fit: cover;"></video>
    </div>
    @endif
    @else
    @if(AttachmentHelper::getAttachmentType($attachment->type) == 'image')
