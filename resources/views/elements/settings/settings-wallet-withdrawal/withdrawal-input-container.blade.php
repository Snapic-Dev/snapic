<div class="input-group mb-3 mt-3 d-block">
    ...
    <div class="ml-2 justify-content-center mt-2 walletCarousel">
        ...
        <button class="btnTwo btn btn-round border ml-2"
            onclick="inputWithdrawalValue(`{{ \App\Providers\PaymentsServiceProvider::getDepositMinimumAmount() + 100 }}`)">
            R${{ \App\Providers\PaymentsServiceProvider::getDepositMinimumAmount() + 100 }},00
        </button>
        <button class="btnThree btn btn-round border ml-2"
            onclick="inputWithdrawalValue(`{{ \App\Providers\PaymentsServiceProvider::getDepositMinimumAmount() + 300 }}`)">
            R${{ \App\Providers\PaymentsServiceProvider::getDepositMinimumAmount() + 300 }},00
        </button>
        <button class="btnFour btn btn-round border ml-2"
            onclick="inputWithdrawalValue(`{{ \App\Providers\PaymentsServiceProvider::getDepositMinimumAmount() + 500 }}`)">
            R${{ \App\Providers\PaymentsServiceProvider::getDepositMinimumAmount() + 500 }},00
        </button>
        <button class="btnFive btn btn-round border ml-2"
            onclick="inputWithdrawalValue(`{{ \App\Providers\PaymentsServiceProvider::getDepositMinimumAmount() + 800 }}`)">
            R${{ \App\Providers\PaymentsServiceProvider::getDepositMinimumAmount() + 800 }},00
        </button>
        <button class="btnSix btn btn-round border ml-2"
            onclick="inputWithdrawalValue(`{{ \App\Providers\PaymentsServiceProvider::getDepositMinimumAmount() + 1000 }}`)">
...
@endforeach
        ...
        <div class="form-group w-100 input-message p-2">
            ...
            <textarea placeholder="{{ __('Bank account, notes, etc') }}" class="form-control" id="withdrawal-message"
                rows="2"></textarea>
...
@endif
    ...
    <div class="p-3 pb-4 mt-4">
        <p class="text-xs alertWitdrawalMsg">
            ...
            Por favor, esteja ciente de que saques só podem ser realizados por indivíduos maiores de <strong>18
                anos</strong>. Qualquer tentativa de saque por <strong>menores de idade</strong> será rejeitada conforme
            nossa <strong>política de segurança</strong>.
        </p>
    </div>
</div>

<script src="{{ asset('C:\Users\Pichau\Documents\Programação\Php\snapic\public\js\pages\settings\withdrawal.js') }}">
</script>
<script>
