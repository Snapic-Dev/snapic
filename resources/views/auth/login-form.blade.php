<form method="POST" action="{{ route('login') }}">
    ...
    <div class="loginHelpers form-group d-flex flex-row-reverse">
        ...
        <div class="pull-right p-1">
            ...
            @else
            <a href="{{ route('password.request') }}" class="text-bold" id="forgotPass-label">{{ __('Forgot Your Password?') }}</a>
            @endif
        </div>
        @endif
    </div>
...
<div class="text-center py-2 p-1">
    <p class="">
        ...
        <!-- <a href="{{ route('register') }}"
                 ...
                 -->
        <a href="{{ route('register') }}" id=" registerLink" class="text-primary text-gradient font-weight-bold" style="cursor: pointer;">{{ __('Sign up') }}</a>
